...
func (bfs *BreadthFirstSearch) Solve() {
  fmt.Println("Starting to solve a maze using Breadth First Search...")
  ...
  for {
    ...
    if bfs.Game.Animate {
      bfs.Game.OutputImage(fmt.Sprintf("tmp/%06d", bfs.Game.NumExplored))
    }
	...
	for _, x := range candidates {
		if 0 <= x.State.Row && x.State.Row < bfs.Game.Height {
			if 0 <= x.State.Col && x.State.Col < bfs.Game.Width {
				if !bfs.Game.Wall[x.State.Row][x.State.Col].wall {
