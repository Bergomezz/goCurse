...
func CleanRoomRandomWalk(room *Room, robot *Robot) {
  ...
  // Set variables
  maxMove := room.Width * room.Height * 5
  stuckCount := 0
  ...
  for moveCount< maxMove && room.CleanedCellCount < room.CleanableCellCount {
    // Generate a random angle in radians
    angle := rand.Float64() * 2 * math.Pi

    // Calculate a direction vector base on angle
    dx := math.Cos(angle)
    dy := math.Sin(angle)

    // Use Bresenhan's line algorithm to move in the direction until hitting a obstacle
    moves := moveAtAngleUntilObstacle(room, robot, dx, dy)
    moveCount += moves

    // If we din't move very much, increment stuck counter and possibly change strategy
    if moves < 3 {
      stuckCount++

      // If we stuck too many time, use A* to find path to nearest dirty cell
    ...
    // Add some adaptive behavior. Scan for dirty cell every once in awhile.
    if moveCount%20 == 0 {
...
func bresenhamLine(x0,y0,x1,y1 int) []Point {
  // Initialize a slice to storage all Points on the line
  var points []Point
  ...
  for {
    ...
    // Check to see if we've reach the endpoint
    if x0 == x1 && y0 == y1 {
      break
    }
    ...
    // If move in x direction, would keep us closer to the ideal line
    if e2 > -dy {
      // If we've reach the endpoint, stop
      if x0 == x1 {
        break
      }
    ...
    // If move in y direction, would keep us closer to the ideal line
    if e2 < dx {
      // If we've reach the endpoint, stop
      if y0 == y1 {
        break
