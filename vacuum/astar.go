...
func Astar(room *Room, start, goal Point) []Point {
  ...
  for pq.Len() > 0 {
    ...
    // If we,ve reach the goal, reconstruct and return the path
    ...
    for _, dir := range directions {
      ...
      if !room.IsValid(neighbor.X, neighbor.Y) || closeSet[neighbor] {
        continue
      }
